...
public:
    ...
    template <class T>
    static bool copyImage(cv::Mat im,T*& pImagePlane,int& width,int& height, int& nchannels);
    template <class T>
...
bool ImageIO::loadImage(const char *filename, T *&pImagePlane, int &width, int &height, int &nchannels)
{
    ...
    pImagePlane = (T*)xmalloc(im.total() * im.elemSize());
    CvmatToPixels(im, pImagePlane, width, height, nchannels);
    return true;
}

template <class T>
bool ImageIO::copyImage(cv::Mat im, T *&pImagePlane, int &width, int &height, int &nchannels)
{
    if (im.data == NULL){
        return false;
    }
    pImagePlane = (T*)xmalloc(sizeof(T) * im.total() * im.elemSize()/im.elemSize1());
    CvmatToPixels(im, pImagePlane, width, height, nchannels);
    return true;
}
...
void ImageIO::CvmatToPixels(cv::Mat& cvInImg, T*& pOutImagePlane, int& width, int& height, int& nchannels)
{
    if (cvInImg.data == NULL) // if allocation fails
        return;
    if (cvInImg.type() != CV_8UC1 &&  cvInImg.type() != CV_8UC2 && cvInImg.type() != CV_8UC3 && cvInImg.type() != CV_8UC4 && cvInImg.type() != CV_32FC3) // we only support three types of image information for now
        return;
    ...
    // check whether the output type is float point
    bool IsFloat = false;
    if (typeid(T) == typeid(double) || typeid(T) == typeid(float) || typeid(T) == typeid(long double))
        IsFloat = true;

    if( cvInImg.type() == CV_32FC3 )
    {
        for (int i = 0; i < height; i++)
        {
            int offset1 = i*width*nchannels;
            float* in_ptr = cvInImg.ptr<float>(i);

            for (int j = 0; j < width*nchannels; j++, in_ptr++)
            {
                if (IsFloat)
                    pOutImagePlane[offset1 + j] = *in_ptr;
                else
                    pOutImagePlane[offset1 + j] = (*in_ptr) * 255.0;
            }
        }
